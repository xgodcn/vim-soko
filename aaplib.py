    ...
    def do_unzip(zfile, dir):
        ...
        def realpath(arcname):
            if dir:
                ...
                return arcname
        msg_info(recdict, "unzip: Extracting %s" % (zfile))
        for arcname, name in [(name, realpath(name)) for name in namelist]:
            msg_info(recdict, "unzip: %s -> %s" % (arcname, name))
