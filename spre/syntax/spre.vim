...
function! s:Setup()
  ...
  let ft_list = {}
  let pos = getpos('.')
  silent g/\v^[#!]%(pre|div|span)\s+\w+>/let ft_list[matchstr(getline('.'), '\v^[#!]%(pre|div|span)\s+\zs\w+>')] = 1
  call setpos('.', pos)
  syntax region MacroPre matchgroup=MacroTag start=/\v^\z([#!])%(pre|div|span)>.*$/ end=/^\z1end\>/ keepend extend fold
  for ft in keys(ft_list)
    ...
    execute printf('syntax region MacroPre matchgroup=MacroTag start=/\v^\z([#!])%(pre|div|span)\s+%s>.*$/ end=/^\z1end\>/ contains=@G%s keepend extend fold', ft, ft)
  endfor
